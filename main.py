...
group_username = "howcanyoumendabrokenheart"
dummy_group = 'seriusinidummy'
...
async def main():
    ...
    async def send_video(document: types.InputDocument, message: str):
        return await client.send_message(
            dummy_group, message=message, file=document, link_preview=True, supports_streaming=True
        )
    ...
    try:
        for i in range(100):
            ...
            if message:
                ...
                try:
                    print('ok' if message.video else 'not ok')
